...
int assert_path(const int *path, const double *distance_matrix,
  ...
  if (check_nnodes != 0 ||
      (tour_length != -1 && check_tour_length != tour_length)) {
    // info message check nodes and print if check_tour_length is the same as
    // tour_length
    ...
    for (int l = 0; l < nnodes; l++)
      DEBUG_COMMENT("assert_path", "path[%d]=%d", l, path[l]);
    printf("check_nnodes: %d\n", check_nnodes);
    printf("check_tour_length: %lf\n", check_tour_length);
