...
void printTabuList(TabuList *list) {
    ...
    while (true) {
        if (list->entries[i].value != -1) {
            DEBUG_COMMENT("tabu::printTabuList()", "Value: %d, Age: %d", list->entries[i].value, list->entries[i].age);
        }
...
void testTabuList() {
    ...
    INFO_COMMENT("tabu::testTabuList()", "Size: %d, Tenure: %d", list->size, list->tenure);
    ...
    printTabuList(list);
    if (areAnyValuesTabu(list, 4, 10, 11, 12) && !areAnyValuesTabu(list, 10, 11, 12, 13)) {
