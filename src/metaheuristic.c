...
{
    //double delta = (cost_new - cost_current) / (cost_current + cost_new) * coefficient;
...
void simulate_anealling(const double *distance_matrix, int *path, int nnodes, double *tour_length, double k_max, int duration)
{
    ...
    for (int k = 0; k < k_max; k++)
    {
        ...
        kick_function(distance_matrix, current_state, nnodes, &current_state_cost, 4);
        ...
        double value = energy_probabilities(old_coefficient_cost, new_state_cost, T, 0.8);
