...
int main(int argc, char **argv) {
  ...
  for (int c_inst = 0; c_inst < args.num_instances; c_inst++) {
    ...
    else if (strcmp(passagges[j], "cplexheu") == 0) {
      instances[c_inst].percentageHF =
          80;  // TODO implement in cmd argument parsing
      instances[c_inst].solver = 4;
      two_opt(distance_matrix, args.nnodes, path,
    ...
    if (args.toplot) {
      ...
      plot(inst->path, args.x, args.y, args.nnodes, title, inst->starting_node);
    }
