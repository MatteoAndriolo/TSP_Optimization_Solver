...
void genetic_setup(GENETIC_SETUP *gen, int ngenerations, int population_size,
                   int windows_size, int grasp_n_probabilities,
...
void genetic_population(GENETIC_POPULATION *popul, GENETIC_SETUP *gen) {
  if (!popul) {
  ...
  init_grasp(popul->grasp_individuals, gen->grasp_probabilities,
             gen->grasp_n_probabilities);
}

void genetic_individual(GENETIC_INDIVIDUAL *indiv, int nnodes, int *path,
                        double tour_lenght) {
  if (!indiv) {
