...
void plot(const int *path, const double *x, const double *y, const int nnodes,
  ...
  fprintf(gnuplotPipe,
          "plot '-' with line, '-' with points, '-' with points pointtype 7 "
          "pointsize 1 lc 'red'\n");
  ...
  fprintf(gnuplotPipe, "%d %d\n", (int)x[path[index_starting_node]],
          (int)y[path[index_starting_node]]);
  fprintf(gnuplotPipe, "e\n");

  // PNG
  // fprintf(gnuplotPipe, "set terminal pngcairo size 800,600 font
